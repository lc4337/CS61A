...
def missing_digits(n):
    ...
    def count(n, pre):
        if n == 0:
            return 0
        return count(n // 10, n % 10) + max(pre - 1 - n % 10, 0)
...
def make_anonymous_factorial():
    ...
    """
    # Y_combinator
    # refer to https://rosettacode.org/wiki/Y_combinator#Python
    return (lambda f: f(f))(lambda f: lambda n: 1 if n < 2 else mul(n, f(f)(sub(n, 1))))
