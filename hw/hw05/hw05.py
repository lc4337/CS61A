...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, pd):
        ...
        if len(wrong_passwords) >= 3:
            return "Too many incorrect attempts. Attempts: " + str(wrong_passwords)
        if pd != password:
            wrong_passwords.append(pd)
...
def repeated(t, k):
    ...
    def helper(t, c, prev):
        ...
        n = next(t)
        if prev == n:
            return helper(t, c - 1, prev)
        return helper(t, k, n)
    return helper(t, k, next(t))
