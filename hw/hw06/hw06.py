...
def is_bst(t):
    ...
    if len(t.branches) == 2:
        return bst_max(t.branches[0]) <= t.label and t.label <= bst_min(t.branches[1]) and is_bst(t.branches[0]) and is_bst(t.branches[1])
    return False
...
def store_digits(n):
    ...
    if n < 10:
        return Link(n)
    link = store_digits(n // 10)

    p = link
    while not p.rest is Link.empty:
        p = p.rest
    p.rest = Link(n % 10)
    return link
