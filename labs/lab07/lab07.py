def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    copy = [list(x) for x in nested_list]
    for lst in copy:
        lst.insert(0, item)
    return copy
...
def make_glookup(class_assignments):
    ...
    def glookup(assignment, score):
        nonlocal all_sum, grade
        all_sum += class_assignments[assignment]
        grade += score
        return grade / all_sum
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n < 2:
        return 1
    return sum(num_trees(i + 1) * num_trees(n - i) for i in range(n))
