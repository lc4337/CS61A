...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        dis = 0
        while nearest_place.bees == [] or dis < self.min_range:
            nearest_place = nearest_place.entrance
            dis += 1
            if dis > self.max_range or nearest_place == beehive:
                return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        #self.place.bees may changed, so we need to make another copy of it
        bees = self.place.bees[:]
...
# END Problem 13
    ...
    def remove_from(self, place):
        # Special handling for queen ants
        # Impostor queen was removed. Queen should remain in the game
        if QueenAnt.real_queen is not self:
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if QueenAnt.real_queen is None:
            QueenAnt.real_queen = self
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self is not QueenAnt.real_queen:
            self.reduce_armor(self.armor)
        else:
            p = self.place.exit
            while p is not None:
                if p.ant not in QueenAnt.st_double and hasattr(p.ant, 'damage'):
                    p.ant.damage *= 2
                    QueenAnt.st_double.add(p.ant)
                if isinstance(p.ant, ContainerAnt):
                    if p.ant.contained_ant not in QueenAnt.st_double and hasattr(p.ant.contained_ant, 'damage'):
                        p.ant.contained_ant.damage *= 2
                        QueenAnt.st_double.add(p.ant.contained_ant)
                p = p.exit 
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self is QueenAnt.real_queen:
            bees_win()
        # END Problem 13
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if hasattr(self, 'direction') and self.direction == 'r':
            destination = None
            if not isinstance(self.place.entrance, Hive):
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if target and target not in ScaryThrower.scared_bees:
            ScaryThrower.scared_bees.add(target)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        cur = self.place
        insects_and_dis = {}
        while cur is not beehive and cur is not None:
            if cur.ant is not None and cur.ant is not self:
                insects_and_dis[cur.ant] = dis
            for insect in cur.bees:
                insects_and_dis[insect] = dis
            dis += 1
            cur = cur.entrance
        return insects_and_dis
        # END Problem OPTIONAL
