...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        hf, hv = formals, vals
        while hf is not nil:
            child.define(hf.first, hv.first)
            hf, hv = hf.rest, hv.rest
        return child
        # END PROBLEM 10
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    t = scheme_eval(expressions.first, env, (expressions.rest is nil))
    if expressions.rest is nil and is_true_primitive(t):
        return t
    return False if is_false_primitive(t) else do_and_form(expressions.rest, env)
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    t = scheme_eval(expressions.first, env, (expressions.rest is nil))
    return t if is_true_primitive(t) else do_or_form(expressions.rest, env)
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    tb = bindings
    while tb is not nil:
        validate_form(tb.first, 2, 2)
        tb = tb.rest
    names, values = bindings.map(lambda x: x.first), bindings.map(lambda x: scheme_eval(x.rest.first, env))
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        formals = target.rest
        validate_formals(formals)
        env.define(target.first, MacroProcedure(formals, expressions.rest, env))
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
